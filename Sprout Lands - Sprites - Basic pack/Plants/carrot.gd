...
# Ambil texture untuk pertumbuhan dan hasil agar hemat memory (di-preload)
var growth_texture = preload("res://Sprout Lands - Sprites - Basic pack/Objects/Farming Plants v2.png")
var harvest_texture = preload("res://Sprout Lands - Sprites - Basic pack/Objects/Farming Plants items v2.png")
...
func _process(_delta: float):
	...
	# ----------------------------------------------------
	# PENGATURAN ANIMASI/SPRITE DUA FILE PNG BERBEDA
	# ----------------------------------------------------
	if stage >= MAX_STAGE:
		# 1. Jika matang (Stage 5): gunakan Farming Plants items v2.png
		plant.texture = harvest_texture
		plant.hframes = 5
		plant.vframes = 14
		plant.frame = 6 # DIUBAH MENJADI 6 (wortel matang di file items)# Frame wortel matang di file items
	else:
		# 2. Jika masih tumbuh (Stage 1-4): gunakan Farming Plants v2.png
		plant.texture = growth_texture
		plant.hframes = 7
		plant.vframes = 33             # Diubah menjadi 33 sesuai inspector kamu
		plant.frame = 16 + (stage - 1) # Diubah dimulai dari 16 (menghasilkan 16, 17, 18, 19)
	# ----------------------------------------------------
...
func _add_harvest() -> void:
	# Jika ingin otomatis masuk tas langsung seperti Corn:
	Game.add_harvest_item("Carrot")
	
	# JIKA INGIN DIJATUHKAN DI TANAH (DROP ITEM):
	# Hapus baris 'Game.add_harvest_item("Carrot")' di atas, dan aktifkan kode spawn drop ini:
	# var drop = preload("res://Scenes/Objects/DropItem.tscn").instantiate()
	# drop.global_position = global_position
	# if drop.has_method("set_item"):
	#     drop.set_item("Carrot")
