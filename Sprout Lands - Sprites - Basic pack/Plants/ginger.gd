...
func _process(_delta: float):
	...
	# ----------------------------------------------------
	# PENGATURAN ANIMASI/SPRITE DUA FILE PNG BERBEDA (GINGER)
	# ----------------------------------------------------
	if stage >= MAX_STAGE:
		...
		plant.hframes = 5
		plant.vframes = 14
		plant.frame = 8 # Menggunakan frame 8 (sesuai inspector hasil Ginger)
	else:
		# 2. Jika masih tumbuh (Stage 1-4): gunakan Farming Plants v2.png
		plant.texture = growth_texture
		plant.hframes = 7
		plant.vframes = 33             # Sesuai Vframes di inspector kamu
		plant.frame = 121 + (stage - 1) # Dimulai dari 121 (menghasilkan 121, 122, 123, 124)
	# ----------------------------------------------------
...
func _add_harvest() -> void:
	# Masuk ke tas/inventory
	Game.add_harvest_item("Ginger")
